...
class CMakeBuild(build_ext):
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        try:
            ...
            cmake_args += [ "-GNinja",  f"-DCMAKE_MAKE_PROGRAM:FILEPATH={ninja_executable_path}"]
        except ImportError:
            pass
        build_args = []
        ...
        subprocess.run(["cmake", ext.source_dir, *cmake_args], cwd=build_temp, check=True)
        subprocess.run(["cmake", "--build", ".", *build_args], cwd=build_temp, check=True)


# The information here can also be placed in setup.cfg - better separation of
# logic and declaration, and simpler if you include description/version in a file.
setup(name="cudnn",version="1.1.0",author="",
    author_email="",description="cudnn_frontend python package",
    long_description="", ext_modules=[CMakeExtension("cudnn")],
    cmdclass={"build_ext": CMakeBuild},zip_safe=False,extras_require={"test": ["pytest>=6.0"]},python_requires=">=3.7")
